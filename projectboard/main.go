...
const (
	...
	perPage = 100
	// tokenEnv is the environment variable that contains the
	...
	// orgNameEnv is the environment variable that contains the
	// name of the GitHub organization to query.
	// e.g. kubernetes
	orgNameEnv = "GITHUB_ORG"
	// projectNameEnv is the environment variable that contains the
	// name of the GitHub project to query.
	// e.g. SIG Auth
	projectNameEnv = "GITHUB_PROJECT"
	// filterLabelEnv is the environment variable that contains the
	// label to filter issues and PRs by.
	// If empty, all issues and PRs will be added to the project.
	// e.g. sig/auth
	filterLabelEnv = "FILTER_LABEL"
)
...
func main() {
	...
	orgName := os.Getenv(orgNameEnv)
	if orgName == "" {
		log.Fatalf("environment variable %q must be set", orgNameEnv)
	}
	projectName := os.Getenv(projectNameEnv)
	if projectName == "" {
		log.Fatalf("environment variable %q must be set", projectNameEnv)
	}
	filterLabel := os.Getenv(filterLabelEnv)
	...
	for _, repo := range repos {
		...
		items, err := client.listIssuesAndPullRequests(ctx, orgName, *repo.Name, filterLabel)
		log.Fatal(err)
